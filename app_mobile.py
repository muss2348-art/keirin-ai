...
if "mobile_race_url_text" not in st.session_state:
    st.session_state.mobile_race_url_text = default_race_url
...
def predict(df: pd.DataFrame, mode: str, odds_dict: dict) -> pd.DataFrame:
    ...
    for t in tickets:
        score = 0
        weights = [1.15, 1.0, 0.75] if mode == "穴モード" else [1.2, 0.95, 0.65]

        for i, r in enumerate(t):
            row = df[df["車番"] == r].iloc[0]
            score += row["評価"] * weights[i]
