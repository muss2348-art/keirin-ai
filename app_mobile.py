...
def extract_rates_from_winticket_row_block(block: str, style: str = "") -> dict:
    """
    WINTICKET基本情報の1選手行から勝率・2連対率・3連対率を抽出する。

    WINTICKET本文は脚質の後ろがだいたい以下の並びになる。
    逃 / 捲 / 差 / マ / 1着 / 2着 / 3着 / 着外 / 勝率 / 2連対率 / 3連対率 / ギヤ倍率 / コメント

    例: 追 0 0 2 1 1 2 7 17 3.6 11.1 37.0 3.93 ...
    この場合、脚質後の9〜11個目が勝率系。
    """
    s = normalize_text(block)
    rates = {"勝率": 0.0, "2連対率": 0.0, "3連対率": 0.0}
    ...
    if style and style in ["逃", "捲", "追", "両", "自"]:
        m = re.search(rf"{re.escape(style)}\s+(.{{0,240}})", s)
        if m:
            tail = m.group(1)
            nums = [safe_float(x, None) for x in re.findall(r"(?<!\d)(\d{1,3}(?:\.\d{1,3})?)(?!\d)", tail)]
            nums = [x for x in nums if x is not None]

            # 脚質後 8個は戦法/着順/着外の整数、9〜11個目が勝率系、12個目がギヤ倍率。
            if len(nums) >= 11:
                cand = nums[8:11]
                gear = nums[11] if len(nums) >= 12 else 0.0
                if all(0.0 <= x <= 100.0 for x in cand) and not (cand[0] > 0 and cand[1] == 0 and cand[2] == 0):
                    # ギヤ倍率らしい数字が後ろにある場合はかなり信用できる
                    if 3.0 <= gear <= 4.5 or any(x > 0 for x in cand[1:]):
                        return {
                            "勝率": normalize_rate_value(cand[0], 0.0),
                            "2連対率": normalize_rate_value(cand[1], 0.0),
                            "3連対率": normalize_rate_value(cand[2], 0.0),
                        }

            # 念のため、脚質後に出てくる小数の「最後から4つ目〜2つ目」を勝率系として拾う。
            # 最後の小数はギヤ倍率のことが多い。
            decimals = [safe_float(x, None) for x in re.findall(r"(?<!\d)(\d{1,3}\.\d{1,3})(?!\d)", tail)]
            decimals = [x for x in decimals if x is not None]
            if len(decimals) >= 4 and 3.0 <= decimals[-1] <= 4.5:
                cand = decimals[-4:-1]
                if all(0.0 <= x <= 100.0 for x in cand):
                    return {
                        "勝率": normalize_rate_value(cand[0], 0.0),
                        "2連対率": normalize_rate_value(cand[1], 0.0),
                        "3連対率": normalize_rate_value(cand[2], 0.0),
    ...
    # 最後の保険としてラベル付きのみ拾う。整数の2/100などは拾わない。
    labeled = extract_rates_from_text_block(s)
    if labeled and any(float(labeled.get(k, 0.0) or 0.0) > 0 for k in RATE_COLUMNS):
        return labeled
    return rates
...
def _desired_rank_counts(max_count: int) -> dict:
    ...
    hot = max(1, int(round(n * 0.18)))
    solid = max(1, int(round(n * 0.28)))
    if hot + solid >= n:
        solid = max(1, n - hot - 1)
    hole = max(1, n - hot - solid)
...
def _build_player_profile_map(current_df: pd.DataFrame) -> dict:
    ...
    for _, row in current_df.iterrows():
        ...
        profiles[car] = {
            ...
            "勝率": _get_player_value(row, ["勝率", "winRate", "firstRate", "firstPlaceRate"], 0.0),
            "2連対率": _get_player_value(row, ["2連対率", "二連対率", "quinellaRate", "doubleRate", "top2Rate"], 0.0),
            "3連対率": _get_player_value(row, ["3連対率", "三連対率", "trioRate", "trifectaRate", "top3Rate"], 0.0),
            "脚質": str(row.get("脚質", "")),
...
def calc_line_break_risk(leader: dict, second: dict, line_size: int):
    ...
    if leader_b >= 12:
        ...
        risk += 1.4
        reasons.append("先行B警戒")
    if leader_h >= 12:
        risk += 2.6
        reasons.append("先行H強")
    elif leader_h >= 9:
        risk += 1.2
        reasons.append("先行H警戒")
...
def _break_side_score(profile: dict, appear_count: int = 0, used_count: int = 0) -> float:
    ...
    # v39.2: 番手評価を強化。番手は千切れ/別線だけでなく2着・3着の柱として扱う
    if line_pos == 2:
        score += 9.0
    elif line_pos == 3:
        score += 4.0

    # v39.2: 千切れ/別線寄りをさらに強化
    score += min(b, 18) * 1.65
    score += min(h, 18) * 1.10
    ...
    # すでに多く使われている車ばかり足さない
    score -= max(0, used_count - 2) * 2.0
    score -= _line_bias_penalty(profile)

    return round(score, 3)


def add_line50_break50_tickets(
    pred_df: pd.DataFrame,
    current_df: pd.DataFrame,
    ticket_type: str,
    max_add: int = 4,
    ...
    """
    v39.2: ライン30%・千切れ70%へ寄せる補正。
    雨の日はライン20%・千切れ80%目安まで別線/展開ズレを強める。
    - 軸は既存予想から維持
    ...
    for car, prof in profiles.items():
        ...
        # v39.2: 同ライン後方ばかり拾わない。雨はさらに千切れ寄り。
        if line_no and axis_line and line_no == axis_line and safe_int(prof.get("ライン順", 0), 0) >= 3:
            score -= 9.0 if is_rain else 7.5

        # v39.2: 番手は評価する。番手はライン残り/千切れ後の2着候補として残す。
        if line_no and axis_line and line_no == axis_line and safe_int(prof.get("ライン順", 0), 0) == 2:
            score += 8.0

        # v39.2: 別線・単騎を優先。通常は30/70、雨は20/80のイメージ。
        if (line_no != axis_line) or prof.get("単騎"):
            score += 13.0 if is_rain else 10.5

        scored.append((car, score))

    scored = sorted(scored, key=lambda x: x[1], reverse=True)
    break_cars = [c for c, _ in scored[:5]]
    ...
    if str(ticket_type) == "2車単":
        ...
        for h in break_cars:
            ...
            # 強い別線/単騎なら頭ズレも1点だけ
            prof = profiles.get(h, {})
            if prof and h != axis1 and (_rate_score(prof) >= 34 or safe_int(prof.get("B", 0), 0) >= 8):
                partner = axis2
                if partner and h not in [axis1, partner]:
                    patterns.append(f"{h}-{axis1}-{partner}")
    ...
    balance_label = "ライン20/千切れ80" if is_rain else "ライン30/千切れ70"
    ...
    if "AI評価" in extra_df.columns:
        extra_df["AI評価"] = 70 if is_rain else 68
    if "期待値" in extra_df.columns:
        extra_df["期待値"] = 110 if is_rain else 107

    extra_df["ライン30千切れ70"] = "雨ON" if is_rain else "ON"
...
def get_df() -> pd.DataFrame:
    ...
    for c in DEFAULT_COLUMNS:
        if c not in df.columns:
            df[c] = 0.0 if c in ["競走得点", "勝率", "2連対率", "3連対率"] else ""
...
with c4:
    if st.button("選手情報を自動取得", use_container_width=True):
        try:
            ...
            st.session_state["widget_ver"] = st.session_state.get("widget_ver", 0) + 1
            st.session_state["message"] = f"選手情報取得成功: {len(players_df)}人 / 勝率系: {int((players_df[["勝率", "2連対率", "3連対率"]].fillna(0).astype(float).sum(axis=1) > 0).sum())}人"
            st.rerun()
        except Exception as e:
    ...
    if st.button("買い目を出す", type="primary", use_container_width=True):
        try:
            ...
            if st.session_state.get("enable_line50_break50", True):
                ...
                selection_info["line50_reason"] = line50_info.get("reason", "")
            else:
