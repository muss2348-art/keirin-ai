...
st.caption("✅ mobile 旧ロジック寄せ_BOX任意版 v35.1（選手取得安定化）")
...
def extract_players_with_regex(text: str, num_riders: int):
    ...
    for m in entry_pattern.finditer(s):
        ...
        if (
            ...
            seen.add(car)
            win_rate = safe_float(m.group(8), 0.0)
            quinella_rate = safe_float(m.group(9), 0.0)
            trio_rate = safe_float(m.group(10), 0.0)
            rows.append({
                "車番": car, "選手名": name, "競走得点": score,
                "勝率": win_rate, "2連対率": quinella_rate, "3連対率": trio_rate,
                "脚質": style
            })
            preview.append({
                "車番": car, "選手名": name, "競走得点": score,
                "勝率": win_rate, "2連対率": quinella_rate, "3連対率": trio_rate,
                "脚質": style, "source": "entry_pattern"
            })
    ...
    return normalize_player_df(df, num_riders), {
        "hit_count": len(df),
        "preview": preview[:12],
    }
...
def normalize_player_df(players_df: pd.DataFrame, num_riders: int) -> pd.DataFrame:
    ...
    for rate_col in ["勝率", "2連対率", "3連対率"]:
        if rate_col not in df.columns:
            df[rate_col] = 0.0
        df[rate_col] = pd.to_numeric(df[rate_col], errors="coerce").fillna(0.0).clip(lower=0.0, upper=100.0)
    ...
    def candidate_quality(row) -> float:
        ...
        if style in ["逃", "捲", "追", "両", "自"]:
            q += 8
        # 勝率系は取れたら品質加点。取れなくても選手取得失敗にはしない。
        if safe_float(row.get("勝率", 0), 0) > 0:
            q += 2
        if safe_float(row.get("2連対率", 0), 0) > 0:
            q += 2
        if safe_float(row.get("3連対率", 0), 0) > 0:
            q += 2
        return q
...
def extract_players_from_json_html(html: str, num_riders: int):
    ...
    win_rate_keys = ["勝率", "winRate", "firstRate", "firstPlaceRate", "firstRatio", "winningRate"]
    quinella_rate_keys = ["2連対率", "二連対率", "quinellaRate", "doubleRate", "top2Rate", "place2Rate"]
    trio_rate_keys = ["3連対率", "三連対率", "trioRate", "trifectaRate", "top3Rate", "place3Rate"]
    ...
    for txt in json_texts:
        try:
            ...
            for d in _walk_json(data):
                ...
                win_rate = _pick_win_rate_from_keys(d, win_rate_keys, 0.0)
                quinella_rate = _pick_win_rate_from_keys(d, quinella_rate_keys, 0.0)
                trio_rate = _pick_win_rate_from_keys(d, trio_rate_keys, 0.0)
                if 1 <= car <= num_riders and is_valid_player_name(name) and 40 <= score <= 130 and style in ["逃", "捲", "追", "両", "自"]:
                    item = {
                        "車番": car, "選手名": name, "競走得点": score,
                        "勝率": win_rate, "2連対率": quinella_rate, "3連対率": trio_rate,
                        "脚質": style, "source": "json_recursive"
                    }
                    rows.append(item)
                    preview.append(item)
        except Exception:
...
def fetch_players_from_winticket(url: str, num_riders: int):
    ...
    if best_df.empty:
        return normalize_player_df(best_df, num_riders), {"source_url": "", "candidate_results": debug_items, "warning": "一部取得または補完取得です"}
